// This needs to be changed to the Write Key for the website Source
const SEGMENT_WRITE_KEY = "your writekey";
...
function getConsentedIntegrations(enabledIntegrations, oneTrustGroupIds) {
  ...
  console.log(
    `getConsentedIntegrations returned: ${consentedIntegrations.map(
      (i) => i.id
    )}`
  );
  ...
  conditionallyLoadAnalytics({
    ...
    if (!destinationPreferences) {
      if (isConsentRequired) {
        console.log(
          `conditionallyLoadAnalytics(): Not loading AJS as consent required and no preferences provided`
        );
        return;
      }
      ...
      if (!analytics.initialized) {
        console.log(
          `conditionallyLoadAnalytics(): loading AJS normally as consent not required and no preferences given`
        );
    ...
    if (isAnythingEnabled) {
      console.log(
        `conditionallyLoadAnalytics(): loading AJS with integrations enabled: ${JSON.stringify(
          integrations
        )}`
      );
